...
for trace in [sys.argv[1]]:
	...
	for algo in labels:
		...
		# if "copa" == algo:
		# 	os.system("tshark -r ./{0}/{1}{2}/log.pcap -T fields -e frame.time_epoch -e frame.len 'ip.{3}==100.64.0.4' > ./{0}/{1}{2}/throughput.csv".format(algo,trace,1,"src"))
		# else:
		# 	os.system("tshark -r ./{0}/{1}{2}/log.pcap -T fields -e frame.time_epoch -e frame.len 'ip.{3}==100.64.0.4' > ./{0}/{1}{2}/throughput.csv".format(algo,trace,1,"dst"))

		if algo == 'copa':
			delays, delayTimes = parse_delay_copa("./{0}/{1}{2}/info.out".format(algo,trace,1))
		else:
			delays, delayTimes = parse_delay(glob("./{0}/{1}{2}/*/".format(algo,trace,1))[0]+"Receiver.out")
		...
		sns.kdeplot(totalThroughput[algo] , color=colors[algo], shade=True, ax = ax1, lw=3)
		sns.kdeplot(totalDelay[algo] , color=colors[algo], label=algo, shade=True, ax = ax2, lw=3)
...
ax1.set_xlim([0,max(totalThroughput[algo])])
ax2.set_xlim([0,800])

#ax2.legend(loc='best')
#ax1.legend(bbox_to_anchor=(1.01, 1), loc='upper center')

ax1.grid()
ax2.grid()
#plt.show()
plt.savefig("copa_pdf_{0}.png".format(trace),dpi=300,bbox_inches='tight')
plt.close()
	...
	# plt.savefig("copa_overall_"+trace+'.png',bbox_inches='tight')