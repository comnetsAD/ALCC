...
void prepare_sock(int sock1)
{
    int on = 1;
#ifdef TCP_NODELAY
    setsockopt(sock1, IPPROTO_TCP, TCP_NODELAY, (void *) &on, sizeof(on));
#endif
#ifdef TCP_NOPUSH
	setsockopt(sock1, IPPROTO_TCP, TCP_NOPUSH, (void *) &on, sizeof(on));
#endif
#ifdef SO_REUSEADDR
	setsockopt(sock1, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof(on));
#endif
#ifdef SO_REUSEPORT
	setsockopt(sock1, SOL_SOCKET, SO_REUSEPORT, (void *) &on, sizeof(on));
#endif
#ifdef SO_SNDBUF
	on = 1000000;
	setsockopt(sock1, SOL_SOCKET, SO_SNDBUF, (void *) &on, sizeof(on));
#endif
}
...
int dataconn()
{
	...
	if (pasv) {
		...
		sock1 = alcc_accept(pasvsock, (struct sockaddr *) &foo, (socklen_t *) &namelen);
    ...
    if (setsockopt(sock1, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse)) < 0)
		...
		if (bind(sock1, (struct sockaddr *) &local, sizeof(local)) < 0) {
		...
		if (connect(sock1, (struct sockaddr *) &sa, sizeof(sa)) == -1) {
...
void do_stor(char *filename, int flags)
{
    ...
    */
    ...
    alarm(0);
    buffer = (char*) malloc(xfer_bufsize);
    ...
    max = (sock1 > stdin_fileno ? sock1 : stdin_fileno) + 1;
	...
	if (!((i = recv(sock1, buffer, my_buffer_size - 1, 0))))
            break;
        bytes_recvd += i;
...
void command_retr(char *filename)
{
	...
	switch (whattodo) {
		...
		case DO_NORMAL:
			...
			offset = 0;
			buffer = (char*) malloc(xfer_bufsize * 2 + 1);
  ...
  offset = 0;
  alarm(control_timeout);
	control_printf(SL_SUCCESS, "226 BBB File transmission successful.");
