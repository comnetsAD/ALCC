	...
	while (!strchr(str, '}')) {
  		if (str[0] != '\n') {
            if ((strstr(str, "directory")) && (strchr(str, '{')) && (directories)) {
                char *tmp;
                if (dir) {
                    dir = dir->next = (struct directory*) malloc(sizeof(struct directory));
                } else {
                    *directories = dir = (struct directory*) malloc(sizeof(struct directory));
                }
                /* avoid memory bug */
                if (! dir) return;
                ...
                create_options(configfile, &(dir->options), NULL);
       			...
       			if (opt) {
       				opt = opt->next = (bftpd_option*) malloc(sizeof(struct bftpd_option));
       			} else {
       				*options = opt = (bftpd_option*) malloc(sizeof(struct bftpd_option));
       			}
            ...
            while (strchr(grp->temp_members, ',')) {
                ...
                if (foo[0] == '@') {
                    if (sscanf(foo + 1, "%i", &uid)) {
                        ...
                        if (!((grpinfo = getgrnam(foo + 1))))
                            continue;
                    if (grp->groups)
                        endg = endg->next = (list_of_struct_group*) malloc(sizeof(struct list_of_struct_group));
                    else
                        grp->groups = endg = (list_of_struct_group*) malloc(sizeof(struct list_of_struct_group));
                    ...
                    endg->grp.gr_mem = (char**) malloc((i + 1) * sizeof(char *));
                    for (i = 0; grpinfo->gr_mem[i]; i++)
                ...
                if (sscanf(foo, "%i", &uid)) {
                    ...
                    if (!((temp = getpwnam(foo))))
                        continue;
                if (grp->users)
                    endp = endp->next = (list_of_struct_passwd*) malloc(sizeof(struct list_of_struct_passwd));
                else
                    grp->users = endp = (list_of_struct_passwd*) malloc(sizeof(struct list_of_struct_passwd));
	...
	while ((str = config_read_line(configfile))) {
		if (strchr(str, '{')) 
            ...
            if (!strcasecmp(str, "global{\n")) {
                ...
                if (usr) {
                    usr = usr->next = (struct user*) malloc(sizeof(struct user));
                } else {
                    config_users = usr = (struct user*) malloc(sizeof(struct user));
                }
                /* avoid memory errors */
                if (! usr)
                {
                ...
                *strchr(usr->name, '{') = 0;
                ...
                if (grp) {
                    grp = grp->next = (group_of_users*) malloc(sizeof(struct group_of_users));
                } else {
                    config_groups = grp = (group_of_users*) malloc(sizeof(struct group_of_users));
                }
                if (! grp)
                {
                    fclose(configfile);
...
char *getoption_directories(struct directory *dir, char *name) {
    ...
    strcat(curpath, "/");
    do {
        bar = (char*) malloc(strlen(dir->path) + 2);
        if (! bar)
           return NULL;
