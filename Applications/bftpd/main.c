...
void handler_sigchld(int sig)
{
	...
	// pid = wait(NULL);
		...
		childpid = (bftpd_childpid*) bftpd_list_get(child_list, i);
...
int main(int argc, char **argv)
{
	...
	if (daemonmode) {
		struct sockaddr_in myaddr, new1;
		...
		my_length = sizeof(new1);
		control_printf(SL_SUCCESS, "Mengal 1.");
		while ((sock = accept(listensocket, (struct sockaddr *) &new1, &my_length))) {
			control_printf(SL_SUCCESS, "Mengla 2.");
			pid_t pid;
			...
			if (sock > 0) {
				...
				if (!pid) {       /* child */
					...
					struct bftpd_childpid *tmp_pid = (bftpd_childpid*) malloc(sizeof(struct bftpd_childpid));
					tmp_pid->pid = pid;
					tmp_pid->sock = sock;
